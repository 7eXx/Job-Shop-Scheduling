...
'''
Questo file testa la procedura greedy per l'assegnazione iniziale della soluzione 
alle macchine rispettando LPT per esempio
'''
...
# funzione che torna la macchina piu' scarica di una lista
def findLightMachine(machines):

    empty = machines[0]
    for m in machines:
        # se trova una macchina piu' scarica aggiorna
        if m.machineLoad() < empty.machineLoad():
            empty = m

    return empty


## implementazione lptGreedy
def lptGreedy(machines, jobs_list):
    ## recupera tutti i task
    all_tasks = []
    ## per tutti i jobs
    for j in jobs_list:
        # per tutti i task nel job
        for t in j.tasks:
            all_tasks.append(t)

    all_tasks.sort(key=lambda task: task.executionTime, reverse=True)

    for t in all_tasks:

        e_machine = findLightMachine(machines)
        e_machine.addSimpleTask(t)
